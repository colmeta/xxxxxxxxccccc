...
export default function IntelligenceView({ session }) {
    ...
    const getIntentColor = (score) => {
        if (score >= 85) return 'rgba(0,255,100,1)'
        if (score >= 60) return 'rgba(255,200,0,1)'
                    ...
                    {results.map(result => {
                        ...
                        return (
                            <div key={result.id} style={{
                                ...
                                borderRadius: '12px',
                                padding: '1.5rem',
                                border: `1px solid ${result.intent_score >= 80 ? 'rgba(0,255,100,0.3)' : 'rgba(255,255,255,0.05)'}`,
                                transition: 'all 0.3s',
                                cursor: 'pointer'
                            }}>
                                {/* Header with Intent */}
